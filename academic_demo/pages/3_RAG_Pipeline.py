...
# Fetch system parameters to show actual configurations in the pipeline visualization
...
st.markdown(
    ...
    <div class="glass-card">
        ...
        <p style="color: #d1d5db; line-height: 1.6;">
            NORAY implements a resilient retrieval architecture. If the dense vector storage (Qdrant) is degraded, the system automatically falls back to sparse lexical indexing (BM25) and conversational memory logs, guaranteeing answer generation without runtime exceptions.
        </p>
    </div>
    """,
...
st.markdown("### 🗺️ Step-by-Step Architecture Pipeline Flow")

steps = [
    {
        "title": "1. Ingestion & Parsing",
        "desc": "Documents (PDF, DOCX, PPTX, Images) are uploaded. The system extracts raw content text using libraries such as <code>pdfplumber</code> and fallback <code>Tesseract OCR</code> engines.",
        "icon": "📤"
    },
    {
        "title": "2. Preprocessing & Character Cleaning",
    ...
    {
        "title": "3. Semantic Character Chunking",
    ...
    },
    {
        ...
        "desc": f"Each text chunk is converted to a dense float vector using embedding provider <b>{provider}</b> with model <b>{embed_model}</b> (vector dimension: <code>{vector_dim}</code>).",
        "icon": "🧬"
    },
    {
        "title": "5. Dual-Indexing & Persistence",
        "desc": f"Generated vectors are loaded into <b>Qdrant Vector DB</b> for semantic searches, and raw text corpus maps are compiled into <b>BM25</b> indices for sparse matches.",
        "icon": "💾"
    },
    {
        "title": "6. Context Retrieval & Fusion",
        "desc": "Incoming user queries trigger parallel dense vector searches and sparse keyword searches. Results are fused using Reciprocal Rank Fusion (RRF).",
        "icon": "🔍"
    },
    {
    ...
    {
        "title": "9. Synthesized Streaming Response",
